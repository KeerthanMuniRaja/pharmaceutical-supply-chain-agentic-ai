...
from utils.auth_db import engine, Base, get_db
...
def seed_db(db: Session):
    """Seed initial roles and root admin if they don't exist"""
    root_role = db.query(Role).filter(Role.name == "root_admin").first()
    if not root_role:
        root_role = Role(name="root_admin", description="Super Administrator")
...
async def login(request: LoginRequest, db: Session = Depends(get_db)):
    ...
    return LoginResponse(
        ...
        user={"username": user.username, "role": role_name, "name": user.full_name or user.username}
    )
...
async def optimize_routes(request: RouteOptimizationRequest):
    ...
    try:
        ...
        # Record optimization to the blockchain for immutability
        try:
            from services.blockchain_service import blockchain_service
            
            # The result dict might have differently named fields, extract carefully
            total_cost = result.get('total_cost_usd', 0)
            savings = result.get('savings_vs_baseline', '0%')
            dest_list = request.destinations
            
            # Use background dispatch if wait is too long, but we just log mock mode instantly
            bc_res = blockchain_service.record_route_optimization(
                depot_id=request.depot_id,
                destinations=dest_list,
                total_cost_usd=total_cost,
                savings_vs_baseline=savings
            )
            logger.info(f"Blockchain record status: {bc_res}")
            
            # Optionally append blockchain tx_hash to the API response
            if bc_res.get("status") in ["success", "mocked"]:
                result["blockchain_tx"] = bc_res.get("tx_hash")
                
        except Exception as bc_e:
            logger.error(f"Failed to record optimization to blockchain: {bc_e}")
...
async def match_inventory(request: InventoryMatchingRequest):
    ...
    try:
        ...
        async def run_matching() -> Dict[str, Any]:
            ...
            loop = asyncio.get_event_loop()
            return await loop.run_in_executor(None, _run)  # type: ignore  # type: ignore
...
async def get_alerts(severity: Optional[str] = None, limit: int = 10):
    ...
    try:
        ...
        async def run_monitoring() -> Dict[str, Any]:
            def _run() -> Dict[str, Any]:
                return agent.generate_alerts(
...
async def execute_workflow(item_id: Optional[str] = None,
    ...
    try:
        ...
        async def run_workflow() -> Dict[str, Any]:
            def _run() -> Dict[str, Any]:
                return workflow.run_workflow(initial_state)
            loop = asyncio.get_event_loop()
            return await loop.run_in_executor(None, _run)  # type: ignore  # type: ignore
